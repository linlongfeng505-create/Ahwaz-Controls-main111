...
async function main() {
  // Use exact English keyword to get the specific product, not generic ones!
  const query = "Clyde A1011 limit switch";
  // cn.bing.com is accessible and we use it to find the exact product
  const searchUrl = `https://cn.bing.com/images/search?q=${encodeURIComponent(query)}`;

  console.log("Searching cn.bing.com for exact product:", searchUrl);
  ...
  try {
    ...
    for (let i = 0; i < imgUrls.length; i++) {
      ...
      try {
        ...
        // As long as it is an image and > 5KB (some real product photos are small)
    ...
    const productData = {
      ...
      deleteImageIds: [8], // 8 was the last one uploaded
