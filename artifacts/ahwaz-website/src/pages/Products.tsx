...
export default function Products() {
  ...
  const { data: categoriesResult, isLoading: categoriesLoading } = useQuery<CategoriesResponse>({
    queryKey: ["product-categories"],
    queryFn: () => fetch("/api/products/categories").then(r => r.json()),
    staleTime: 30_000,
  });

  const dynamicCategories = categoriesResult?.categories ?? [];
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  >
                    <input
                      ...
                      className="w-full sm:w-64 pl-9 pr-8 py-2 text-sm border border-border rounded-sm bg-background focus:outline-none focus:border-accent transition-colors"
                    />
                  ...
                  {selectedCategory && (
                    <button
                      onClick={() => handleCategoryChange("")}
                      className="hidden sm:inline-flex items-center gap-1 text-xs text-accent hover:underline font-mono"
                    >
                ...
                {selectedCategory && (
                  <button
                    onClick={() => handleCategoryChange("")}
                    className="sm:hidden inline-flex items-center gap-1 text-xs text-accent hover:underline font-mono"
                  >
                    <X className="w-3 h-3" /> Clear filter
                  </button>
                )}
                ...
                <>
                  <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                    {products.map((product, i) => (
                      ...
                      >
                        <Link href={`/${encodeURIComponent(product.category.replace(/\s+/g, '-'))}/${encodeURIComponent(product.name.replace(/\s+/g, '-'))}`}>
