...
async function main() {
  const query = "site:alibaba.com Clyde A1011 limit switch";
  const searchUrl = `https://cn.bing.com/images/search?q=${encodeURIComponent(query)}`;

  console.log("Searching cn.bing.com specifically targeting alibaba for:", searchUrl);
  ...
  try {
    ...
    // Filter to ONLY accept Alibaba CDNs to ensure it's a real product
    const alibabaUrls = imgUrls.filter(u => u.includes('alicdn.com') && !u.includes('placeholder'));
    console.log(`Found ${alibabaUrls.length} Alibaba candidate images.`);

    if (alibabaUrls.length === 0) {
      console.log("No alibaba images found, falling back to first 5 images.");
      alibabaUrls.push(...imgUrls.slice(0, 5));
    }

    let validImageDataUrl = null;

    for (let i = 0; i < alibabaUrls.length; i++) {
      const url = alibabaUrls[i];
      ...
      try {
        ...
        if (arrayBuffer.byteLength > 10000) {
          console.log(`Success! Downloaded Alibaba exact product image of size ${arrayBuffer.byteLength} bytes.`);
    ...
    const productData = {
      ...
      deleteImageIds: [9, 8], // clean up placeholders
    ...
    if (updateRes.ok) {
      ...
      console.log("Product ID 8 updated successfully with ALIBABA EXACT product photo!");
